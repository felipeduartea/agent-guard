...
def install(home, clients=None, dry_run=False):
    ...
    if dry_run:
        return {'dry_run':True,'scope':'all local sessions','configs':{c:str(p[0]) for c,p in plan.items()},
                'guard_directory':str(root),'key_value':'never read by installer'}
    stamp=datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')
    backup=root/'backups'/stamp
    backup.mkdir(parents=True,mode=0o700)
    names=('guard.py','hook.py','set_key.py','README.md','INTEGRATIONS.md')
    ...
    try:
        ...
        manifest={'installed_at':stamp,'backup':str(backup),'scope':'all local sessions',
